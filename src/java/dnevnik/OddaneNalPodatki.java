    ...
    static {
    	if(Sessions.getCurrent().getAttribute("ID_uporabnika") != null && Sessions.getCurrent().getAttribute("ID_naloge") != null){
            ...
            {   
                ...
                Object[] paramPregled = {};   
                s = db.Statement(Boolean.TRUE, "SELECT id, uporabnik_id, aktivna FROM nal_" + idnal + ";", paramPregled);    //Kreiraj sql, ki vrne vsrico za izbrano ucno pripravo     
                ResultSet rs = s.executeQuery ();				    
                vrstice = new ArrayList<OddaneNal>();
                //Vrtimo zanko po vseh vrsticah, ki jih statement vrne
                while (rs.next()) 
                {                   
                    OddaneNal tmp = new OddaneNal(rs.getInt("id"), rs.getString("uporabnik_id"), rs.getInt("aktivna"), idnal);
                    vrstice.add(tmp);
                }
                rs.close ();
                db.close ();
